...
class AircraftDisruptionEnv(gym.Env):
                ...
                for j in range(3, self.columns_state_space, 3):
                    ...
                    if not np.isnan(flight_dep):
                        if flight_dep < current_time_minutes:
                            # Skip this flight, it is in the past and is considered cancelled
                            if DEBUG_MODE:
    ...
    def step(self, action=None):
        ...
        while not agent_acted:
            ...
            # If no conflicts are found, advance the timestep without agent intervention
            if len(pre_action_conflicts) == 0:
                next_datetime = self.current_datetime + self.timestep
                if next_datetime >= self.end_datetime:
                    # If the next time step goes beyond the recovery period, terminate the episode
                    terminated = True
                    truncated = False
                    processed_state = self.process_observation(self.state)
                    return processed_state, 0, terminated, truncated, {}

                # No conflicts and no termination, so advance time and reset the state
                self.current_datetime = next_datetime
                self.state = self._get_initial_state()

                # Check again for conflicts after advancing time
                post_action_conflicts = self.get_current_conflicts()
                if DEBUG_MODE:
                    print(f"Advancing time, checking conflicts again. Current time: {self.current_datetime}")
                    print(f"Conflicts after time advance: {post_action_conflicts}")
                continue  # Skip agent action for this timestep and loop again

            # If conflicts exist, proceed with the logic where the agent needs to take action
    ...
    def _calculate_reward(self, resolved_conflicts, remaining_conflicts, action):
        ...
        if delay_penalty > RESOLVED_CONFLICT_REWARD:
