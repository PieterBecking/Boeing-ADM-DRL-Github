...
class AircraftDisruptionEnv(gym.Env):
    ...
    def process_observation(self, state):
        """Processes the observation by flattening and replacing NaN with a dummy value.
        
        Args:
            state (np.ndarray): The current state of the environment.

        Returns:
            dict: A dictionary containing the processed state.
        """
        ...
        # Return only the state as the observation
        return {'state': state_flat}
    ...
    def step(self, action_index):
        ...
        # Process the state into observation (no action mask in observation)
    ...
    def reset(self, seed=None, options=None):
        ...
        if DEBUG_MODE:
            ...
            print(f"Type of processed state: {type(processed_state)}")  # Debugging statement
    ...
    def get_action_mask(self):
        ...
        Returns:
            np.ndarray: A binary array indicating which actions are valid (1) and which are not (0).
        """
        valid_flight_actions = self.get_valid_flight_actions()
        valid_aircraft_actions = self.get_valid_aircraft_actions()
        ...
        for flight_action in valid_flight_actions:
            for aircraft_action in valid_aircraft_actions:
                if flight_action == 0:
                    # Only allow (flight_action=0, aircraft_action=0) for 'no action'
